...
const INITIAL_MESSAGE: ChatMessage = {
  id: 0,
  role: "assistant",
  text: "Merhaba! Okul, duyurular, yemek listesi ve etkinlikler hakkında size yardımcı olabilirim.",
};

/**
 * AI Veli Asistanı — mock sohbet arayüzü.
 * Gerçek AI entegrasyonu yoktur; gönderilen mesaja örnek (demo) yanıt döner.
 */
export function ParentAiAssistant({ suggestions }: ParentAiAssistantProps) {
  ...
  const sendMessage = (text: string) => {
    ...
    setMessages((prev) => [
      ...
      { id: prev.length, role: "user", text: trimmed },
      {
        id: prev.length + 1,
        role: "assistant",
        text: "Bu bir demo yanıtıdır. Yapay zeka asistanı yakında gerçek verilerinizle yanıt verecek.",
      },
    ]);
    setInput("");
  };
  ...
  return (
    <GlassCard className="ai-gradient flex flex-col border-accent/20">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-lg font-semibold text-content">AI Veli Asistanı</h2>
          <p className="text-xs text-muted">
            Okul, duyurular, yemek listesi, etkinlikler ve öğrencinizin gelişimi
            hakkında sorularınızı sorun.
          </p>
        </div>
      </div>
